...
export default function DeleteProductButton({ slug, id, productName }: Props) {
  ...
  async function handleDelete() {
    setLoading(true)
    setError(null)
    const result = await deleteProduct(slug, id)
    if (result.error) {
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-sm text-gray-600 dark:text-gray-400">
        Delete <span className="font-medium text-gray-900 dark:text-white">{productName}</span>? This cannot be undone.
      </p>
