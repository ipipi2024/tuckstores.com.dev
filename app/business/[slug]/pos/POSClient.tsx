...
function fmtQty(quantity: number, unitSnapshot: string): string {
  if (unitSnapshot === 'unit') return String(quantity)
  return `${quantity.toFixed(3)} ${unitSnapshot}`
}

function fmtStock(stock: number, baseUnit: string): string {
  if (stock <= 0) return 'Out of stock'
  if (baseUnit === 'unit') return `${stock} in stock`
  return `${stock.toFixed(3)} ${baseUnit} in stock`
}
...
export default function POSClient({ products, currencyCode, completeSale, customers, slug }: Props) {
  ...
  // Used for weight/volume products: both initial add and editing.

  const [pendingProduct, setPendingProduct] = useState<{ product: Product; editingId?: string } | null>(null)
  ...
  // Badge uses line-item count, not sum of quantities (avoids showing fractions)
  const cartCount = cart.length

  // ── Cart mutations ─────────────────────────────────────────────────────────

  function addToCart(product: Product, qty?: number) {
    // Weight/volume products: require explicit qty via modal
    if (product.measurement_type !== 'unit') {
      if (qty == null) {
        openQtyModal(product)
        return
      }
      // qty provided (from modal confirm)
      setCart((prev) => {
        const existing = prev.find((i) => i.product_id === product.id)
        if (existing) {
          // replace quantity with the newly entered value
          return prev.map((i) =>
    ...
    // Unit products: tap to add 1, existing behaviour
    setCart((prev) => {
      const existing = prev.find((i) => i.product_id === product.id)
      if (existing) {
  ...
  function openQtyModal(product: Product, editingId?: string) {
    const currentQty = editingId
      ? cart.find((i) => i.product_id === editingId)?.quantity
      : undefined
    setPendingProduct({ product, editingId })
    setQtyInput(currentQty != null ? String(currentQty) : '')
    setQtyError(null)
  ...
  function handleQtyConfirm() {
    ...
    if (!qtyInput.trim() || isNaN(parsed) || parsed <= 0) {
      setQtyError('Enter a valid positive quantity')
      return
    }

    const qty = Math.round(parsed * 1000) / 1000
    const { product, editingId } = pendingProduct

    if (qty > product.stock) {
      setQtyError(`Only ${fmtStock(product.stock, product.base_unit).replace(' in stock', '')} in stock`)
      return
    }

    if (editingId) {
      // Update quantity of existing cart item; qty = 0 path not reachable (caught above)
      setCart((prev) =>
        prev.map((i) => (i.product_id === editingId ? { ...i, quantity: qty } : i))
      )
    } else {
  ...
  const qtyModal = pendingProduct ? (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="bg-white dark:bg-neutral-900 rounded-2xl shadow-xl p-6 w-80 max-w-[90vw] space-y-4">
        <div>
          <h2 className="text-base font-semibold text-gray-900 dark:text-white">
            {pendingProduct.editingId ? 'Edit quantity' : 'Enter quantity'}
          </h2>
        ...
        <div>
          <label className="block text-xs font-medium text-gray-600 dark:text-gray-400 mb-1.5">
            Quantity ({pendingProduct.product.base_unit})
          </label>
          <input
            ref={qtyInputRef}
            type="number"
            ...
            onKeyDown={(e) => e.key === 'Enter' && handleQtyConfirm()}
            placeholder="e.g. 0.500"
            className="w-full px-3 py-2.5 rounded-xl border border-gray-300 dark:border-neutral-700 bg-white dark:bg-neutral-900 text-gray-900 dark:text-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
          />
          {qtyError ? (
            <p className="mt-1.5 text-xs text-red-500 dark:text-red-400">{qtyError}</p>
          ) : pendingProduct.product.stock > 0 ? (
            <p className="mt-1.5 text-xs text-gray-400 dark:text-neutral-500">
              {fmtStock(pendingProduct.product.stock, pendingProduct.product.base_unit)}
            </p>
          ) : null}
        </div>
        ...
        <div className="flex gap-3">
          <button
            ...
            className="flex-1 py-2.5 text-sm font-medium text-gray-600 dark:text-gray-300 rounded-xl border border-gray-200 dark:border-neutral-700 hover:bg-gray-50 dark:hover:bg-neutral-800 transition-colors"
          >
            Cancel
          </button>
          <button
          ...
          >
            {pendingProduct.editingId ? 'Update' : 'Add to cart'}
          </button>
        </div>
      </div>
    </div>
          ...
          {filteredProducts.length === 0 ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3">
              {filteredProducts.map((product) => {
                ...
                const isMeasurable = product.measurement_type !== 'unit'
                return (
                  <button
                    key={product.id}
            ...
            {cart.length === 0 ? (
              ...
              <>
                <div className="divide-y divide-gray-50 dark:divide-neutral-800 max-h-80 overflow-y-auto">
                  {cart.map((item) => {
                    const isMeasurable = item.unit_snapshot !== 'unit'
                      ...
                      <div key={item.product_id} className="flex items-center gap-3 px-4 py-3">
                        <div className="flex-1 min-w-0">
                          ...
                          <p className="text-xs text-gray-400 dark:text-neutral-500 tabular-nums">
                            {fmt(item.unit_price)}{isMeasurable ? `/${item.unit_snapshot}` : ' each'}
                          </p>
                        </div>
                        {isMeasurable ? (
                          /* Measurable: show quantity with unit, edit button, remove button */
                          <div className="flex items-center gap-1.5 shrink-0">
                            <span className="text-sm font-medium text-gray-900 dark:text-white tabular-nums">
                              {fmtQty(item.quantity, item.unit_snapshot)}
                            </span>
                            <button
                              onClick={() => product && openQtyModal(product, item.product_id)}
                              title="Edit quantity"
                              className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-400 dark:text-neutral-500 hover:text-indigo-600 dark:hover:text-indigo-400 hover:bg-gray-100 dark:hover:bg-neutral-800 transition-colors"
                            >
                              <Edit2 size={12} />
                            </button>
                            <button
                              onClick={() => removeFromCart(item.product_id)}
                              className="w-7 h-7 rounded-lg flex items-center justify-center text-gray-300 dark:text-neutral-600 hover:text-red-500 dark:hover:text-red-400 transition-colors"
                            >
                              <Trash2 size={12} />
                            </button>
                          </div>
                        ) : (
                          /* Unit: existing +/- buttons */
